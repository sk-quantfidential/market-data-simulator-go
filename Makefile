# Market Data Simulator Go - Makefile
...
build: ## Build the market data simulator binary
	@echo "Building market data simulator..."
	go build -o market-data-simulator ./cmd/server

clean: ## Clean build artifacts
	@echo "Cleaning..."
	rm -f market-data-simulator server
	go clean -testcache
...
test-files: ## Show test files
	@echo "Test files in market-data-simulator-go:"
	@find . -name "*_test.go" -exec echo "  {}" \;

# Status check
status: ## Check current test status
	@echo "=== Market Data Simulator Go Test Status ==="
	@echo ""
